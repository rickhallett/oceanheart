...
Staging seed: `charming-albatross-632`, tenant `jd75twx1tbhws9agbs2rqeg9kn8e5237`. Second invocation returned `created:false`. The public sample remains available explicitly at `/app?demo=1`, including on authenticated deployments. It mounts only browser-local fictional state and preserves the demo query during navigation. Never wire production to staging data or treat sample payment states as provider receipts. WorkOS production requires its separate billing activation and configuration.
