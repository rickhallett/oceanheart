...
- `audience`: exact WorkOS client ID
...
The verifier accepts only compact bearer JWTs with an RS256 header and a bounded key ID. `jose` verifies the RSA signature, issuer, audience and expiry. The verified payload must contain non-empty `sub`, `sid` and integer `exp` claims. JWKS payloads are size-bounded and restricted to public RSA signing keys; symmetric/private key material and algorithm ambiguity are rejected. A missing key causes one bounded refresh for normal WorkOS rotation. Network, parsing and optional session-status failures time out and fail closed.
...
The focused test creates ephemeral RSA keys, serves their public JWKS from a task-owned loopback HTTP server through a test-only network adapter, and signs tokens with `jose`. The real verification library and network fetch are exercised. It covers valid replay to one durable effect; authenticated run/draft/trace reads; cross-identity denial; auth-free and malformed denial before effects; expiry, issuer, audience, environment, signature and algorithm confusion; key rotation; active-session revocation; and bounded JWKS/status stalls. No actual WorkOS keys, users, sessions or credentials are fixtures.
...
The existing Studio AuthKit integration is unchanged. WorkOS documents access-token validation with `jose`, the JWKS endpoint and signed session claims in [Sessions](https://workos.com/docs/user-management/sessions/introduction); its OAuth guidance gives the supported `createRemoteJWKSet`/`jwtVerify` issuer-and-audience pattern in [OAuth 2.0](https://workos.com/docs/integrations/oauth).
