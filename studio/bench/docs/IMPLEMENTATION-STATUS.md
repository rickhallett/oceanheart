...
| HE-02 | Real `jose` signature/claim verification feeds controller-registry identity resolution and client-scoped run/draft/trace access; the dedicated bridge is live on loopback, while the intended human's first real WorkOS session remains outstanding |
| HE-03 | Actual Pi restricted tools and disabled discovered resources tested; the private host now separates `studio-runtime` from the app OS identity and keeps the bridge on loopback |
| HE-04 | Clean private builder uses a distinct unprivileged service account and built an exact Studio export without credentials; the c0001 release path passes only an explicit app environment allowlist and never the controller token |
| HE-05 | Three clean operator-only guests and private proxy state inspected; one dedicated c0001 Convex development deployment and WorkOS sandbox are now live, controller-inspected and bound. Exact target evidence is private; real-session isolation proof remains outstanding |
...
| HE-08 | Durable local recovery plus remote supervisor stop/restart/replay preserved one synthetic draft and one job/effect/reservation; the c0001 bridge uses the same minimal-guest supervisor, while VM-boot reinstallation remains operator-owned |
| HE-09 | Git export verified; scoped clone/push/PR integration not implemented |
| HE-10 | Standalone Studio switching verifies source/digest/data target before the atomic pointer and preserves exact rollback; c0001 uses the same private router path, with exact hosted release evidence retained outside Git |
...
The accepted Studio staging backend remains shared and is deliberately ineligible. A controller-authorized live operation has now created and re-inspected one dedicated c0001 Convex project/development deployment and one WorkOS sandbox, with exact IDs and receipt retained privately. The existing Studio schema/functions have been deployed only to that dedicated development deployment using an expiring scoped deploy key, and the WorkOS sandbox has one exact callback for the private VM URL. No user, invitation, production target, billing change or real record was created. Placeholder values in `.env.example` remain non-evidence.

The server-side authenticated runtime boundary adds a Clara start/inspect path over the existing Pi/SQLite runtime. It derives client and hashed actor identity from a verified principal plus the controller registry, never from request client fields, and repeats authorization for run, draft and trace reads. The concrete WorkOS verifier validates RS256 signatures, exact issuer/audience, expiry, subject and session ID against a bounded HTTPS JWKS adapter; the server-configured environment is not taken from request or token claims. A controller-supplied active-session adapter can additionally reject revoked sessions and fails closed on timeout. The standalone bridge binds only to loopback under the runtime OS identity, while the exact-source Studio application runs under a distinct app identity and forwards its server-obtained WorkOS token. Hosted prepare/retry proof still requires the intended human to sign in and the controller to bind that exact subject; URL access alone is never authorization.
