...
export class PiWorkflowRuntime {
 ...
 async startRun(request:RunRequest):Promise<Job>{validateInput(request.input);if(request.clientId!==request.input.clientId||!request.actor||!request.idempotencyKey||request.idempotencyKey.length>128||(request.configurationVersion!==undefined&&!/^[a-zA-Z0-9._-]{1,100}$/.test(request.configurationVersion))||(request.configurationReleaseId!==undefined&&!/^[0-9a-f]{64}$/.test(request.configurationReleaseId)))throw Error('INVALID_REQUEST');const snapshot=JSON.parse(JSON.stringify(request));return this.resumeSession(request.clientId,this.store.enqueue(snapshot).id);}
