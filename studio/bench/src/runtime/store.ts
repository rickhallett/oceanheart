...
export class JobStore {
  ...
  effect(client:string,id:string,token:string):ClaraResult{return this.transaction(()=>{const job=this.fenced(client,id,token);const prior=this.db.prepare('SELECT json FROM effects WHERE job=?').get(id);if(prior)return JSON.parse(prior.json as string);const result=calculateInvoice(job.request.input,'draft-'+id);for(const line of result.lines){const existing=this.db.prepare('SELECT job FROM reservations WHERE client=? AND session=?').get(client,line.sessionId);if(existing&&existing.job!==id)throw Error('SESSION_ALREADY_DRAFTED');this.db.prepare('INSERT OR IGNORE INTO reservations VALUES(?,?,?)').run(client,line.sessionId,id);}this.db.prepare('INSERT INTO effects VALUES(?,?)').run(id,JSON.stringify(result));this.event(job,'draft_persisted');return result;});}
