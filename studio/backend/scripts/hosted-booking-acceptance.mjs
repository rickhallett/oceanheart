...
process.umask(0o077);
const directory=await mkdtemp(join(tmpdir(),"studio-booking-acceptance-"));
...
try {
  ...
  const startsAt=Number(process.env.STUDIO_BOOKING_TEST_START_MS);
  assert.ok(Number.isSafeInteger(startsAt)&&startsAt>Date.now()&&startsAt+86400000<=8640000000000000,"Explicit valid future fixture start in UTC milliseconds required");
  report.runId=randomUUID();await save();
  const tenantA=await alice.mutation("tenants:create",{name:`Booking acceptance ${report.runId.slice(0,8)}`,requestKey:`${report.runId}-owner-practice`});
  report.records.push({table:"tenants",id:tenantA});await save();
  const tenantB=await bob.mutation("tenants:create",{name:`Booking isolation ${report.runId.slice(0,8)}`,requestKey:`${report.runId}-outsider-practice`});
  report.records.push({table:"tenants",id:tenantB});await save();
  const window={tenantId:tenantA,from:startsAt-3600000,to:startsAt+86400000};
  assert.equal((await alice.query("bookings:list",window)).items.length,0,"Choose an empty fixture day before creating bookings");
  report.timeZonesBefore=[{tenantId:tenantA,timeZone:(await alice.query("tenants:list",{})).find(t=>t._id===tenantA).timeZone??null},{tenantId:tenantB,timeZone:(await bob.query("tenants:list",{})).find(t=>t._id===tenantB).timeZone??null}];
  report.cleanup="This run created isolated practice fixtures; their timezone and records are intentionally retained. Existing practices are unchanged. Use exact recorded IDs if later cleanup is reviewed; no deletion/reset or restoration of an unset timezone is implemented.";
  ...
  await bookingWorkflowChecks({alice,bob,viewer:client("viewer"),anonymous:client(),tenantA,tenantB,viewerIdentity:accounts.viewer.identity,clientForOwner:async()=>client("owner"),prefix:randomUUID(),startsAt,check:name=>{report.checks.push(name);console.log(`PASS ${name}`);},record:async(table,id,tenantId)=>{report.records.push({table,id,tenantId});await save();}});
...
finally{report.finishedAt=new Date().toISOString();await save();console.log(`Booking acceptance ${report.status}; non-secret record manifest: ${directory}`);}
