...
process.umask(0o077);
const directory=await mkdtemp(join(tmpdir(),"studio-enquiry-acceptance-"));
...
try {
  ...
  const startsAt=Number(process.env.STUDIO_ENQUIRY_TEST_START_MS);
  ...
  const tenantA=await alice.mutation("tenants:create",{name:`Enquiry acceptance ${report.runId.slice(0,8)}`,requestKey:`${report.runId}-owner-practice`});
  report.records.push({table:"tenants",id:tenantA});await save();
  const tenantB=await bob.mutation("tenants:create",{name:`Enquiry isolation ${report.runId.slice(0,8)}`,requestKey:`${report.runId}-outsider-practice`});
...
finally{report.finishedAt=new Date().toISOString();await save();console.log(`Enquiry acceptance ${report.status}; non-secret record manifest: ${directory}`);}
