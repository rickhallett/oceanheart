...
export default defineSchema({
  knowledgeAccessGrants: defineTable({tenantId:v.id("tenants"),identity:v.string(),capability:v.union(v.literal("read"),v.literal("contribute")),active:v.boolean(),createdBy:v.string(),createdAt:v.number(),revokedAt:v.optional(v.number())}).index("by_tenant_identity",["tenantId","identity"]).index("by_identity",["identity"]),
  knowledgeUploads: defineTable({tenantId:v.id("tenants"),actor:v.string(),storageId:v.id("_storage"),title:v.string(),provenance:v.string(),format:v.union(v.literal("text"),v.literal("markdown"),v.literal("pdf"),v.literal("docx")),status:v.union(v.literal("pending"),v.literal("processing"),v.literal("ready"),v.literal("failed")),errorCode:v.optional(v.union(v.literal("UNSUPPORTED_FORMAT"),v.literal("FILE_TOO_LARGE"),v.literal("INVALID_TEXT"),v.literal("UNREADABLE_SCAN"),v.literal("EXTRACTION_FAILED"))),targetSourceId:v.optional(v.id("knowledgeSources")),expectedRevision:v.optional(v.number()),sourceId:v.optional(v.id("knowledgeSources")),requestKey:v.string(),payload:v.string(),createdAt:v.number(),updatedAt:v.number()}).index("by_tenant_request",["tenantId","requestKey"]).index("by_tenant_status",["tenantId","status"]).index("by_source",["sourceId"]).index("by_target_source",["targetSourceId"]),
  ...
  workflowBriefs: defineTable({tenantId:v.id("tenants"),actor:v.string(),version:v.literal(1),title:v.string(),outcome:v.string(),reviewNotes:v.string(),references:v.array(proposalReference),status:v.union(v.literal("draft"),v.literal("accepted"),v.literal("changes_requested")),revision:v.number(),requestKey:v.string(),creationPayload:v.string(),createdAt:v.number(),reviewedAt:v.optional(v.number()),reviewedBy:v.optional(v.string()),reviewRequestKey:v.optional(v.string()),reviewPayload:v.optional(v.string())}).index("by_tenant",["tenantId"]).index("by_tenant_request",["tenantId","requestKey"]),
  knowledgeSources: defineTable({tenantId:v.id("tenants"),title:v.string(),provenance:v.string(),format:v.union(v.literal("text"),v.literal("markdown")),audience:v.literal("owner"),archived:v.boolean(),deletedAt:v.optional(v.number()),revision:v.number(),currentVersionId:v.optional(v.id("knowledgeVersions")),approvedVersionId:v.optional(v.id("knowledgeVersions")),createdBy:v.string(),requestKey:v.string(),creationPayload:v.string(),lastAction:v.optional(v.string())}).index("by_tenant_archived",["tenantId","archived"]).index("by_tenant_archived_deleted",["tenantId","archived","deletedAt"]).index("by_tenant_request",["tenantId","requestKey"]),
