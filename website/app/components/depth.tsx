...
export function DepthPage({ title, label, children, practice }: { practice?: Practice; title: string; label: string; children: ReactNode }) {
  return <div className="editorial-page editorial-light"><SiteNav practice={practice} /><main className="depth-page"><p className="eyebrow">{label}</p><h1>{title}</h1>{children}</main><Footer practice={practice} /></div>;
}
