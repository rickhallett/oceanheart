  ...
  return <div className="possibilities-explorer">
    <div className="possibilities-scenarios" role="tablist" aria-label="Choose an everyday difficulty" aria-orientation="vertical">
      {journeys.map((item, index) => <button key={item.id} ref={node => { tabs.current[index] = node; }}
        ...
        <span className="scenario-number">0{index + 1}</span><span>{item.label}</span><span aria-hidden="true">↗</span>
      </button>)}
    </div>
    <div className="possibilities-journey" role="tabpanel" tabIndex={0} id="selected-journey" aria-labelledby={`scenario-${journey.id}`}>
      <p className="p-kicker">{journey.short}</p>
      ...
      <div className="journey-demo">
        <div className="journey-demo-header"><span className="p-kicker">Walk through the possibility</span><span>Illustrative example</span></div>
        ...
        <div className="journey-step-content" aria-live="polite" aria-atomic="true">
          <p>{current.detail}</p>
          <div className="journey-example-note"><span className="example-owner">{current.owner}</span><h4>{current.screen}</h4><p>{current.note}</p><div className="example-action"><span aria-hidden="true">◌</span>{current.action}</div></div>
        </div>
...
export function ConnectionExplorer() {
  ...
  return <div className="connection-explorer">
    <div className="connection-choices" role="group" aria-label="Explore tools you already use">{connections.map((item, index) => <button key={item.id} aria-pressed={selected === index} onClick={() => setSelected(index)}>{item.label}</button>)}</div>
