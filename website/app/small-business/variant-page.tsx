...
function CTA({ v, secondary = false }: { v: Variant; secondary?: boolean }) {
 return <a className={'sb-button' + (secondary ? ' sb-button-secondary' : '')} href={bookingLink('digital').href}>{v.cta}<span aria-hidden="true">↗</span></a>;
}
...
export default function VariantPage({v}: {v:Variant}) {
 const index = variants.findIndex(item => item.slug === v.slug);
 return <main id="main-content" className={`sb-page sb-theme-${v.theme} sb-layout-${v.layout}`}>
  <a className="sb-skip" href="#possibilities">Skip to the details</a>
  <ReviewBar slug={v.slug} index={index}/>
  <header className="sb-header"><a className="sb-wordmark" href="/">oceanheart.ai</a><nav aria-label="Page navigation"><a href="#possibilities">What we could do</a><a href="#questions">Questions</a><a href="#conversation">Let’s talk <span aria-hidden="true">↗</span></a></nav></header>
  ...
  <footer className="sb-footer"><a href="/" className="sb-wordmark">oceanheart.ai</a><span>Rick Hallett · Practical AI guidance</span><a href="https://dev.oceanheart.ai/">Design, engineering & AI ↗</a></footer>
  <nav className="sb-bottom-review" aria-label="Explore other page variants"><a href={pathFor(variants[(index+14)%15].slug)}>← Previous</a><a href="/small-business">All 15 directions</a><a href={pathFor(variants[(index+1)%15].slug)}>Next →</a></nav>
 </main>;
}
