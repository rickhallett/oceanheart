...
function StarIcon({ filled }: { filled: boolean }) {
 return <svg aria-hidden="true" focusable="false" width="1em" height="1em" viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} style={{display:'inline-block',verticalAlign:'-0.15em',flexShrink:0}}><path d="m12 3 2.8 5.7 6.3.9-4.6 4.4 1.1 6.3-5.6-3-5.6 3 1.1-6.3L2.9 9.6l6.3-.9L12 3Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /></svg>;
}
...
export function ReviewBar({slug,index}: {slug:string;index:number}) {
 ...
 return <aside className="sb-review" aria-label="Design review controls"><a href="/small-business"><ArrowIcon direction="left" /> All directions</a><span className="sb-review-title">{String(index+1).padStart(2,'0')} / 15 · {variants[index].name}</span><div><a href={pathFor(variants[(index+14)%15].slug)} aria-label="Previous variant"><ArrowIcon direction="left" /></a><a href={pathFor(variants[(index+1)%15].slug)} aria-label="Next variant"><ArrowIcon direction="right" /></a><button aria-pressed={saved.includes(slug)} onClick={()=>toggle(slug)}><StarIcon filled={saved.includes(slug)} /> {saved.includes(slug)?'Saved':'Shortlist'}</button><button onClick={()=>setHidden(true)}>Hide controls</button></div>{notice && <p role="status">{notice}</p>}</aside>;
}
