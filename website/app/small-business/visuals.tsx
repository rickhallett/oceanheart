...
export function Visual({ variant: v, small = false }: { variant: Variant; small?: boolean }) {
 ...
 if(v.art === 'quote') return <div className="sb-quote-visual" aria-label="Illustrative journey from job notes to a checked quote">
  ...
  <div className="sb-quote"><span className="sb-label">02 / Draft for review</span><h3>Garden gate replacement</h3><dl><dt>Scope</dt><dd>Replace gate; retain posts</dd><dt>Finish</dt><dd>Awaiting customer choice</dd><dt>Price</dt><dd>Add your agreed rates</dd></dl><span className="sb-check">Your approval comes next <ArrowIcon /></span></div>
 ...
 if(v.art === 'enquiry') return <div className="sb-enquiry"><div className="sb-inputs"><span>Email enquiry</span><span>Website message</span><span>Telephone notes</span></div><div className="sb-connector" aria-hidden="true"><ArrowIcon direction="down" /></div><div className="sb-enquiry-card"><span className="sb-label">Example / Details to check</span><h3>One clear next step.</h3><p>What’s needed?<br/>Where and when?<br/>What do we still need to ask?</p><div className="sb-check">A person reviews the reply <ArrowIcon /></div></div></div>;
 ...
 if(v.art === 'documents') return <div className="sb-document-visual"><span className="sb-label">An example, using familiar files</span><div className="sb-file-row"><span>Document</span><span aria-hidden="true"><ArrowIcon direction="down-right" /></span></div><div className="sb-file-row"><span>Spreadsheet</span><span aria-hidden="true"><ArrowIcon direction="right" /></span></div><div className="sb-file-row"><span>Email notes</span><span aria-hidden="true"><ArrowIcon /></span></div><div className="sb-document-result"><span className="sb-label">Your next step</span><h3>A summary<br/>you can check.</h3><p>Source material stays the reference.</p></div></div>;
